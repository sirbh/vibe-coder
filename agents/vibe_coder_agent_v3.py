...
def apply_updates(updated_files: List[Dict[str, str]]) -> None:
    ...
    """
    return apply_updates_to_container("nextjs-hlrso6","my-next-app",updated_files)
...
system_prompt = read_file_as_string("prompts/system_v3.text").replace("{{PROJECT_FILES_HERE}}",json.dumps(get_project_files(),indent=2))
